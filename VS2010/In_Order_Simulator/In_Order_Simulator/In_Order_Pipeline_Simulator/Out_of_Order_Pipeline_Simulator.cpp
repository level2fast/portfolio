...
/*if(LittleCmprPipeLineSim.branchTargetFound(newStateParam->pc)) //if program counter is found in the Branch target buffer
	{
		newStateParam->IFID.instr	= newStateParam->instrMem[branchTargetBuffer.front().targetAddress]; //use predicted instruction
		newStateParam->pc           = branchTargetBuffer.front().targetAddress;	//use predicted program counter
		newStateParam->IFID.pcPlus1 = branchTargetBuffer.front().targetAddress;	//use predicted program counter for pc plus one  
		branchTargetBufferEntryFound = true; //set control line state for status of BTB  entry to on
		//set predicted branch to true
		return;
	}
	else*/ 
...
void LittleComputer::MemWbStage(stateType *newStateParam)
{
	...
	//else if(curOpcode == BEQ && newStateParam->EXMEM.branchCtrlLineState == false && branchTargetBufferEntryFound) //branch is not taken, but this is a predicted branch instruction
	//{//mispredicted branch
	//	//kill fetched instruction
	//	//send control signals for noop instructions
	//	newStateParam->IFID.instr  = NOOPINSTRUCTION;
	//	newStateParam->IDEX.instr  = NOOPINSTRUCTION;
	//	newStateParam->EXMEM.instr = NOOPINSTRUCTION;
	//	//restart fetch at other target
	//	branchCtrlLine.branch    = newStateParam->EXMEM.branchCtrlLineState; //set the current state of the branch ctrl line data path to "on" 
	//	//delete entry from target buffer
	//	branchTargetBuffer.erase(bufferEntryPtr);  
	//}
...
void LittleComputer::WbEndStage(stateType *newStateParam)
{
	...
	if(curOpcode == LW)
	{
		...
		//use the destination register which is first 16 bits for I-type
...
int LittleComputer::MemDataOperation(stateType *newStateParam, int curOpcode)
{
	...
	{
		newStateParam->MEMWB.writeData = newStateParam->dataMem[effectiveAddress]; //use the effective address to read from register file and initialize register B
	}
...
{
	//branchTargetBufferType temp;
	//for (std::list<branchTargetBufferType>::const_iterator buffer = branchTargetBuffer.begin(); buffer != branchTargetBuffer.end(); ++buffer)
	//  {
	//	if(buffer->pc == pc)
	//	{
	//		temp.pc = buffer->pc;
	//		temp.targetAddress = buffer->targetAddress;
	//		branchTargetBuffer.push_front(temp);
	//		bufferEntryPtr = buffer;
	//		branchTargetBuffer.erase(buffer);
	//		return true;
	//	}
	//  }
	//
